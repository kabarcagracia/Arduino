...
void publish_data(){
  ...
  if(datadevice != "")
  {
    if(SendCommand("", "AT+HTTPDATA= " + String(datadevice.length()) + ",1000", "DOWNLOAD", "ERROR"))
    {
      if(SendCommand("", datadevice, "OK", "ERROR"))
      {
        if(SendCommand("", "AT+HTTPACTION=1", "200", "ERROR"))
        {
          publicado = true;
          MONITOR.println("publicado");
        }
...
void check_SD(){
  MONITOR.println("SD revisada");
}

bool http_start(){
  delay(2000);
  if(SendCommand("", "AT+NETOPEN?", "OK", "ERROR"))
  {
    if(SendCommand("", "AT+HTTPINIT", "OK", "ERROR"))
    {
      if(SendCommand("", "AT+HTTPPARA=\"URL\",\"http://34.230.28.13:3000/api/gps_send2\"", "OK", "ERROR"))
      {
        if(SendCommand("", "AT+HTTPPARA=\"CONTENT\", \"application/json\"", "OK", "ERROR"))
        {
          if(SendCommand("", "AT+HTTPPARA=\"ACCEPT\", \"/*/\"", "OK", "ERROR"))
          {
            return true;
          }
        }
